...
function rankAndChooseUsers(users: User[], assignments: ClinicAssignment[], values: (string | number | boolean)[][]){
  ...
  let allDates = assignments.map((clinic) => clinic.AvailabilityDict);
  ...
  do{
    ...
    assignments.forEach((clinic)=>{
      // Visit each clinic and assign one user at a time
      // This prevents one clinic from consuming all of the user availability
      for (let d in clinic.AvailabilityDict) { // TODO: cache dates
        let date = clinic.AvailabilityDict[d];
        // Default:
        ...
        if(numAssigned > 0) break; // Yield to another clinic
      }
    });
  } while (numAssigned != 0);
